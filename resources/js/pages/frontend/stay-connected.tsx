...
export default function StayConnected({ product }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('stayconnected.store'), {
            onSuccess: () => {
            ...
            onError: (errors) => {
                toast.error('Failed to subscribe to stay connected!');
            },
        });
    };
                ...
                <div className="max-w-2xl mx-auto bg-gray-50/50 border border-gray-100 rounded-[2.5rem] p-8 md:p-16 text-center shadow-sm">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6 text-left max-w-md mx-auto">
                        ...
                        <button
                            ...
                            disabled={processing}
                            className={cn(
                                "w-full py-4 rounded-xl font-bold text-sm transition-all shadow-lg cursor-pointer",
                                processing
                                    ? "bg-gray-400 text-white cursor-not-allowed"
                                    : "bg-blue-600 text-white hover:bg-blue-700 shadow-blue-500/20"
                            )}
                        >
                            {processing ? 'Submitting...' : 'Get Text Alerts'}
                        </button>
                    </form>
