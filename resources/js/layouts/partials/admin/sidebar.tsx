...
const sidebarItems: SidebarItem[] = [
...

    {
        ...
        icon: Users,
        isCurrent: (currentRoute) =>
            currentRoute.startsWith('/admin/users-track'),
    },
    {
...
export const AdminSidebar = React.memo<AdminSidebarProps>(
    ({ isCollapsed, activeSlug }) => {
        ...
        const shouldUseSlugFallback =
            activeSlug === 'dashboard' || activeSlug === 'admin-users';
        ...
        const isActiveItem = (item: SidebarItem): boolean => {
            if (shouldUseSlugFallback) {
                return item.title === 'Overview'
                    ...
                    : item.title === 'User Track' &&
                          activeSlug === 'admin-users';
            }
            ...
            return item.subItems.some(
                (subItem) => subItem.isCurrent?.(currentRoute) ?? false,
            );
        };
        ...
        const sidebarContent = (mobile = false) => (
            <div
                className={cn(
                    'fixed top-0 left-0 z-50 flex h-full flex-col border-r border-gray-200 bg-white shadow-sm',
            ...
            >
                ...
                <div
                    className={cn(
                        'mt-auto border-t border-gray-100 pt-6',
                        !mobile && isCollapsedView && 'w-full',
                    )}
                >
                    <button
                        type="button"
        ...
        return (
            <>
                <button
                    type="button"
                    className="fixed top-4 left-4 z-40 rounded-lg border border-gray-200 bg-white p-2 text-gray-700 shadow-sm md:hidden"
                ...
                >
                    <button
                        type="button"
                        className="absolute top-4 right-4 rounded-lg border border-gray-200 bg-white p-2 text-gray-700 shadow-sm"
                        onClick={() => setIsMobileOpen(false)}
